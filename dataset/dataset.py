...
class Dataset:
    ...
    def create_dataset_csv(self):
        ...
        for dataset_folder in dataset_folders:
            dataset = pd.read_csv(
                self.datasets_path / dataset_folder / "labels.csv",
                encoding="ISO-8859-1",
            )
        ...
        self._export_dataset(self.complete_dataset, self.dataset_filename)
    ...
    def split_train_test(self, train_size: float = 0.8):
        ...
        train_dataset = self.complete_dataset[
            self.complete_dataset["filename"].isin(filenames_train)
        ]
        test_dataset = self.complete_dataset[
            self.complete_dataset["filename"].isin(filenames_test)
        ]
    ...
    def load_hf_dataset(self):
        ...
        self.dict_int2str, self.dict_str2int, self.num_classes = (
            self._create_dict_labels()
        )
    ...
    def _reconstract_objects(self, batch):
        return {
            "tokens": ast.literal_eval(batch["tokens"]),
            "ner_tags_str": ast.literal_eval(batch["ner_tags_str"]),
        }
    ...
    def _format_hf_dataset(self, dataset: pd.DataFrame):
        ...
        return dataset.groupby(["idx_sentence", "filename"], as_index=False).agg(
            {"tokens": list, "ner_tags_str": list}
        )
